...
package com.davita.documents
{
	 ...
	 *	@since  13.11.2007
	 */
	public class CourseWrapperReviewTest extends MovieClip
	{
		/* ============= */
		/* = Variables = */
		/* ============= */
		 ...
		 *	@constructor
		 */
		public function CourseWrapperReviewTest()
		{	
		...
		/* = debugger functions = */
		/* ====================== */
		private function startDebugger(event:ContextMenuEvent):void
		{
        ...
        private function createForm (event:ContextMenuEvent) {
            var issueInput:TextArea = new TextArea();
            var submitButton:Button = new Button();
            issueInput.width = 200;
            issueInput.height = 200;
            submitButton.label = "SUBMIT";

            var filename:String = xmlPages[currentPage].@source.split("/")[1];
            var myLoadedSWF = queue.rawContent as MovieClip;
            var frameNum:Number = myLoadedSWF.currentFrame;

            // init the form
            var reviewForm:Form = new Form("What's the problem?");
            reviewForm.autoSize = true;
            reviewForm.subFormHeading("Submit the issue using this form");
            this.addChild(reviewForm);

            // init FormDataManager
            var myFormDataManager:FormDataManager = new FormDataManager(FlValueParser);
            myFormDataManager.addTrigger(submitButton, handlerDataCollection);

            reviewForm.formDataManager = myFormDataManager;

            reviewForm.dataSource = [{label:"issue", items:issueInput, id:"issue", source:issueInput}, {label:"", items:submitButton}];
        }

        private function handlerDataCollection (e:FormDataManagerEvent) {
		...
		private function addMenuItems():void
		{
			...
			showReviewItem.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT, createForm);
		}
